function [object_tracks] = trackgen(K,measmodel,motionmodel,sensormodel,birthmodel)
...
%       birthmodel: a structure array specifies the birth model (Gaussian mixture
%                   density) parameters --- (1 x number of birth components)
%           w: weights of mixture components
%           x: mean of mixture components
%           P: covariance of mixture components
%OUTPUT:object_tracks: a structure array specifies object tracks --- (number of tracks x 1)
...
for k = 1:K
    ...
    nb = poissrnd(sum([birthmodel.w]));
    for i = 1:nb
        %randomly sample birth component
        idx = find(rand<cumsum([birthmodel.w]/sum([birthmodel.w])),1,'first');
        n = n+1;
        ...
        ifalive = 1;
        while ifalive && (xpos>=range_c(1,1))&&(xpos<=range_c(1,2))&&(ypos>=range_c(2,1))&&(ypos<=range_c(2,2))&&(time+1<=K)
